...
GLuint LoadTexture(const char *targaFilename, GLuint textureUnit, bool mipmap) {
	...
	GLuint textureId = LoadTexture(pixels, width, height, textureUnit, true, mipmap); // Targa is BGR
	delete [] pixels;
	return textureId;
}
			...
			depthPixels(depthPixels), width(width), height(height), pixelScale(pixelScale) {
				...
				for (int j = 0; j < height; j++)		// row
					for (int i = 0; i < width; i++) {	// column
						vec3 v = Normal(i, j);
						*n++ = 127.5f*(v[0]+1);			// red in [-1,1]
						*n++ = 127.5f*(v[1]+1);			// grn in [-1,1]
						*n++ = 255.f*v[2];				// blu in [0,1]
					}
		}
